## inspirational file, delete after finishing project ###
...
# Add a new column "city" based on the row-numbering
allcities_privatrooms_topbottom <- allcities_privatrooms_topbottom %>%
  mutate(city = case_when(
    row_number() <= 200 ~ "Amsterdam",
    row_number() <= 400 ~ "Berlin",
    row_number() <= 600 ~ "Brussel",
    row_number() <= 800 ~ "London",
    row_number() <= 1000 ~ "Paris"
  ))

# Calculate the total number of reviews per 100 cheapest & 100 most expensive rooms in the city
reviews_summary <- allcities_privatrooms_topbottom %>%
  group_by(city) %>%
  summarize(
    Cheapest_100 = sum(number_of_reviews[is_expensive == 0]),
    Expensive_100 = sum(number_of_reviews[is_expensive == 1])
  )

# Print results for each city
cat("Results:\n")
for (i in 1:nrow(reviews_summary)) {
  cat("City: ", reviews_summary$city[i], "\n")
  cat("Total Reviews for Cheapest 100: ", reviews_summary$Cheapest_100[i], "\n")
  cat("Total Reviews for Most Expensive 100: ", reviews_summary$Expensive_100[i], "\n\n")
}




############################# start simplifying code with advanced programming
